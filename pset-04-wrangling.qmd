...
population <- as_tibble(population) |>  # convert to tibble
  ...
  select(-state) |>                         # remove state id (numbers) column
  ...
  mutate(
    year = parse_number(year),             # remove "POP_" and keep numeric year
...
regions <- map_df(1:nrow(regions), function(i) {
  tibble(
    ...
    region = unlist(regions$region[[i]]),   # convert the list element to scalar
    region_name = regions$region_name[i]    # character vector, no unlist needed
  )
})
...
cases_raw <- resp_body_json(response, simplifyVector = FALSE) |> (\(x) do.call(rbind, x))()
...
kable(cases_summary, caption = "Total COVID-19 Cases by Month and Year (2020–2021)")
```
